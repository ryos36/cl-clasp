...
(pushnew :ryos-fastcgi-patch *features*)
(provide "fastcgi")
...
(defun euc-jp-byte-length (string)
  "Calculate the amount of bytes needed to encode a string."
  (declare (type string string)
           #'*optimize*)
  (let ((length (length string))
    ...
    length))
; 
(defun write-stdout-unicode (u-str)
  (fcgi_write_stdout u-str (euc-jp-byte-length u-str)))
...
(defun write-stdout (data)
  ...
  (do* ((chunksize 65536)
    ...
    (let* ((to-write (min (length s) chunksize))
	   (out-ustr (subseq s 0 to-write))
	   (out-ustr-length (euc-jp-byte-length out-ustr)))
      (fcgi_write_stdout out-ustr out-ustr-length)
